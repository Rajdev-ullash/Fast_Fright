...
const ManageOrder = () => {
  ...
  useEffect(() => {
    fetch(
      "https://immense-headland-98077.herokuapp.com/api/getParcelUser?email=" +
        loggedInUser.email
    )
      .then((res) => res.json())
    ...
    <div>
      <div className="row">
        <div className="col-md-12" style={{ backgroundColor: "#f0f1f7" }}>
          {manageOrderData.map((ride) => (
            <div className="d-flex justify-content-around my-3 py-2 rounded ride-list-container shadow-sm ">
              <div>
...
//     const [manageOrderData, setMangeOrderData] = useState([]);
//     useEffect(() => {
//         fetch('https://immense-headland-98077.herokuapp.com/api/getParcelUser')
//         .then(res => res.json())
